...
";
        ...
        try {
            ...
            if (env('STAFF_NOTIFY_EMAIL')) {

                $staffEmailBody = "
STAFFING OFFICE CALL OFF ALERT
...
";
                ...
                Mail::raw($staffEmailBody, function ($mail) use ($name) {
                    $mail->to(env('STAFF_NOTIFY_EMAIL'))
                        ->subject("Staffing Call Off Alert - $name");
                });
            }
...
Route::match(['get', 'post'], '/voice', function (Request $request) {
    ...
    $from = urlencode($request->input('From') ?? 'unknown');
...
<Response>
    ...
    <Record
        maxLength="60"
        playBeep="true"
        recordingStatusCallback="https://calloff-app-1.onrender.com/voice-recording?from=' . $from . '"
...
Route::match(['get', 'post'], '/voice-recording', function (Request $request) {
    ...
    $from = $request->query('from') ?? 'unknown';
